...
public class DrawUtils {
    ...
    public static void lineWithThickness(MatrixStack matrices, float startX, float startY, float endX, float endY, int z, int startColor, int endColor, float thickness) {
        ...
        bufferBuilder.vertex(matrix, startX + px, startY + py, z).color(sR, sG, sB, alpha).next();
        bufferBuilder.vertex(matrix, endX + px, endY + py, z).color(eR, eG, eB, alpha).next();
        bufferBuilder.vertex(matrix, endX - px, endY - py, z).color(eR, eG, eB, alpha).next();
        bufferBuilder.vertex(matrix, startX - px, startY - py, z).color(sR, sG, sB, alpha).next();
    ...
    public static void drawArrowWithThickness(MatrixStack matrices, float startX, float startY, float endX, float endY, int z, int startColor, int endColor, float thickness, float arrowHeadSize) {
        ...
        bufferBuilder.vertex(matrix, endX, endY, z).color(red, green, blue, alpha).next();
        bufferBuilder.vertex(matrix, arrowHeadX1, arrowHeadY1, z).color(red, green, blue, alpha).next();
        bufferBuilder.vertex(matrix, arrowHeadX2, arrowHeadY2, z).color(red, green, blue, alpha).next();
