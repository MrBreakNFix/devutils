...
public final class EventManager {
        ...
        events.spliterator().forEachRemaining(listener -> {
            ...
            if(listener.event == event.getClass()){
                try {
                    ...
                    Dev.LOGGER.error("Error invoking event " + event.getClass().getSimpleName() + " for listener " + listener.object.getClass().getSimpleName());
                }
            }
        });
    ...
    public void trigger(PacketEvent.Sent sent) {
        events.spliterator().forEachRemaining(listener -> {
            if(listener.event == PacketEvent.Sent.class){
                try {
                    listener.method.setAccessible(true);
                    listener.method.invoke(listener.object, sent);
                } catch (Exception e) {
                    Dev.LOGGER.error("Error invoking event " + sent.getClass().getSimpleName() + " for listener " + listener.object.getClass().getSimpleName());
                }
            }
        });
    }
